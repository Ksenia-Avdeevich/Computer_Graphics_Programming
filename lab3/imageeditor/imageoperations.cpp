...
QImage ImageOperations::adjustContrast(const QImage& img) {
...
QVector<int> ImageOperations::calculateHistogram(const QImage& img, int channel) {
...
QVector<uchar> ImageOperations::createEqualizationTable(const QVector<int>& hist, int totalPixels) {
...
QImage ImageOperations::balanceRGB(const QImage& img) {
    QImage result(img.size(), img.format());
    int totalPixels = img.width() * img.height();

    QVector<uchar> lutR = createEqualizationTable(calculateHistogram(img, 0), totalPixels);
    QVector<uchar> lutG = createEqualizationTable(calculateHistogram(img, 1), totalPixels);
    QVector<uchar> lutB = createEqualizationTable(calculateHistogram(img, 2), totalPixels);
...
QImage ImageOperations::balanceHSV(const QImage& img) {
    QImage result = img.copy();
    int totalPixels = img.width() * img.height();
    ...
    QVector<uchar> lut = createEqualizationTable(hist, totalPixels);
...
QImage ImageOperations::enhanceSharpness(const QImage& img) {
    QImage result = img.copy();
    int kernel[3][3] = {
        { 0, -1,  0 },
        {-1,  5, -1 },
